...
class Array:
    ...
    @staticmethod
    def zeros_like(arr):
        return Array.zeros(arr.shape)
    
    @staticmethod
    def ones_like(arr):
        return Array.ones(arr.shape)
    ...
    def maximum(self, arr, axis=None):
        if axis is None:
            return max(self._flatten(), arr.flatten())
        else:
            new_data = []
            for i in range(self.shape[axis]):
                new_data.append(max(self._get_axis(i, axis), arr._get_axis(i, axis)))
            return Array(new_data)

    def maximum(self, arr, axis=None):
            if axis is None:
                return min(self._flatten(), arr.flatten())
            else:
                new_data = []
                for i in range(self.shape[axis]):
                    new_data.append(min(self._get_axis(i, axis), arr._get_axis(i, axis)))
                return Array(new_data)
    ...
    @staticmethod
    def randint(low, high, shape=None):
        if shape is None:
            data = []
            for _ in range(shape[0]):
                row = []
                for _ in range(shape[1]):
                    if len(shape) > 2:
                        row.append(Array.randint(low, high, shape=shape[2:]).data)
                    else:
    ...
    def uniform(low, high, shape=None):
        ...
        for _ in range(shape[0]):
            ...
            for _ in range(shape[1]):
                if len(shape) > 2:
                    row.append(Array.uniform(low, high, shape=shape[2:]).data)
                else:
    ...
    def rand(shape=None):
        ...
        for _ in range(shape[0]):
            ...
            for _ in range(shape[1]):
                if len(shape) > 2:
                    row.append(Array.rand(shape=shape[2:]).data)
                else:
    ...
    def randn(shape=None):
        if isinstance(shape, tuple) and len(shape) > 1:
            ...
            for _ in range(shape[0]):
                ...
                for _ in range(shape[1]):
                    if len(shape) > 2:
                        row.append(Array.randn(shape=shape[2:]).data)
                    else:
